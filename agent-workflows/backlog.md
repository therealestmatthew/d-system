...
Preflights the repo, shows the top priority phases, and orients on one without claiming it.

Named `backlog` rather than `resume` because Claude Code already owns `/resume` as a built-in.

## Modes

Parse this workflow's invocation arguments for a phase id and a mode flag. Either may appear alone,
together, or not at all.

| Invocation | Behaviour |
|---|---|
| No arguments | Orient on the recommendation, **stop and ask**. The default. |
| `phase-idea-01` | Orient on that phase instead of the recommendation, stop and ask. |
| `--claim` | Orient, **claim** the phase, then stop and ask before working. |
| `--go` | Orient, claim, and **begin the first action** without pausing. |
| `phase-idea-01 --go` | Same, on a named phase. |
...
If the invocation names a phase, use that one instead of the recommendation — the owner has already
decided.
...
## 5. Ask in batches, one batch at a time

**Every question raised during orientation goes through a structured question capability**, not
prose in the reply. A question buried in a paragraph gets one answer covering three things, or none
at all; a structured question returns a decision per question, attributable and unambiguous.

- **Batch up to four questions at a time** and **send one batch at a time**. Wait for the answers,
  then decide whether a further batch is warranted. Answers change what is worth asking next: a
  decision to cut a scope bullet retires every question underneath it.
...
**Default** — ask whether to proceed, through the structured question capability as step 5 requires,
and stop there. Do not set `status: active`. Do not set the `agent` field. Claiming should follow the
owner's decision, not precede it.
