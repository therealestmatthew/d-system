...
Records a parked idea, or moves an existing one along, through the sanctioned writer.
...
Read the invocation's arguments as the idea. Distil a **title** — one line, specific enough to be
recognisable in a list of a hundred — and write the **body** as the idea in full.
...
`--body` let the shell evaluate a command substitution before the writer ever saw the text. Create
the file directly (never via a shell heredoc, which has the same quoting problem) with the title as
its first line and the body as everything after it:
...
record it anyway — triage is a downstream pass, not a gate at the door
(`docs/04-decisions/ADR-010-idea-staging.md`).
