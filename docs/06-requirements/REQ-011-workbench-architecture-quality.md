...
| R29 | The frontend lock table admits two independent phases on different surfaces. `sys-ui` is decomposed in `systems.yaml` along seams observed in the code, and every `phase-arch-*` phase declares the narrower system it actually touches. | Run `src.governance.backlog.collisions()` over the `phase-arch-*` set and confirm the concurrency ceiling rises above 2. Confirm each new system id names a surface that exists in the tree, not a wished-for boundary. Confirm the coarse `sys-ui` id is retired rather than left as a third alias. |
