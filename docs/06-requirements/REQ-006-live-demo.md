...
| R10 | A terminal panel hosts up to four terminal sessions as tabs (the backend session registry separately bounds concurrent sessions to six across all panels — REQ-007 W17); each tab is an independent shell over its own websocket, and switching tabs or collapsing the region never terminates a session — scrollback and running processes survive. | Playwright-driven browser verification: start a long-running command in tab 1, switch to tab 2 and back, collapse and re-expand the region, and assert tab 1's scrollback and process are intact; assert a fifth tab cannot be opened. |
