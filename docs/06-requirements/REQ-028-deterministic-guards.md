...
| R04 | The pre-commit hook refuses a commit when the governance check fails, and still runs the private-content check | In a scratch worktree with the hook active, stage a stale-catalog change and commit: refused, with the governance error printed. Regenerate and commit: accepted. A staged private identifier fixture is still refused |
| R05 | The completion-edit instructions name the catalog regeneration: `GOV-017` merge-gate step 6 and `PROMPT-037` item 4(iii) both say to run `--catalog` and commit the result with the edit | Read both passages. A completion edit made without the regeneration fails R01's check before it can be committed (R04) |
...
| R10 | The merge gate names the new checks: a `READY` carries the output of R07's and R08's checks, and a nonzero R07 result proceeds only with the owner's recorded sign-off for the named tests | Read `GOV-017`'s merge gate and `PROMPT-037` item 4 |
