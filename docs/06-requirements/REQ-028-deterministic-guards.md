...
7. **A plan and its registered phases can drift apart.** Of the 40 plans in `docs/01-plans/` that
   ...
   guards work.
8. **A phase can enter the queue without a plan review.** The owner ruled on 2026-09-23 that a plan
   gets a `GOV-018` review record before G3, and that a phase entering `next_up` without one is
   refused, with the phases queued then exempt ("S3 next_up rule refuses, grandfathers current 19";
   idea `000394`). Nothing checks it: of the 19 phases in `next_up` on `dev` at `f1b891d`, only the
   ten `PLAN-039` phases reviewed in `2026-09-23-plan-039` appear in any review record.
...
| R10 | The merge gate names the new checks: a `READY` carries the output of R07's and R08's checks, and a nonzero result from either proceeds only with the owner's recorded sign-off naming the tests or lines it accepts. Once `phase-dgov-06`'s deliverables-diff check exists, `READY` also carries its output, as a report that does not block | Read `GOV-017`'s merge gate and `PROMPT-037` item 4 |
...
| R12 | The governance check exits 1 when a phase in `next_up` has no review record under `docs/08-governance/reviews/` that lists it in `target.phases` or `target.later_added_phases`, has `status: dispositioned`, and has no finding whose current (last) disposition is `escalated-g3`. The 19 phases in `next_up` on `dev` at `f1b891d` are exempt: `phase-part-03`, `phase-cap-08`, `phase-irs-14`, `phase-irs-11`, `phase-idg-01`, `phase-idg-11`, `phase-dgov-01`, `phase-idg-12`, `phase-irs-05`, `phase-irs-07`, `phase-irs-13`, `phase-irs-08`, `phase-irs-09`, `phase-irs-15`, `phase-agx-03`, `phase-irs-10`, `phase-irs-02`, `phase-irs-12`, `phase-irs-17` | Fixtures: a `next_up` phase with no record (1); with a dispositioned record (0); with a record whose finding's last disposition is `escalated-g3` (1); with an `open` record (1); a grandfathered phase with no record (0). The repository's result recorded |
| R13 | `GOV-017` and the pre-commit hook's operations document state that a failing hook is never bypassed with `--no-verify`: the session commits the fix, or, if the check itself is wrong, stops and reports | Read both passages |
