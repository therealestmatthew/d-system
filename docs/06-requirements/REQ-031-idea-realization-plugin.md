...
- R01 does not require publication, a version-bump procedure, or a marketplace file in this
  repository: the plugin is loaded with `--plugin-dir` during the build and joins a marketplace
  elsewhere later (PLAN-048 D2, owner at G3).
...
- The plugin lives in this repository, for private use, with no marketplace file here: the owner
  ruled at G3 that it will be included in a marketplace elsewhere later; until then it is loaded
  with `--plugin-dir`.
