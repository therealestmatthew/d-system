...
**One document contradicted this entry and was corrected on 2026-09-16**, after the owner reviewed
the replacement wording: `.claude/commands/session-close.md`'s "This command is owner-only, on
purpose" section, which told an agent it "must never invoke this" and that nothing "should be
restructured to make it agent-reachable". It now states the three conditions above and keeps the
protection that actually matters — an agent never completes a phase on its own judgement that the
work looks finished.

**`.claude/skills/checkpoint/SKILL.md` does not contradict this entry**, and was not changed. An
earlier draft of this entry said it did; that was wrong. Its rule is "Never set a phase's `status`
to `complete`. Only `session-close` does that, and only after its own sub-agent review" — a
statement about *which procedure* completes a phase, not about *who invokes* it. That remains
exactly correct under this decision. The file is also generated by
`tools/generate_agent_workflows.py` and is not hand-editable.
...
## The generated ideas-and-backlog page is kept, not declined — 2026-09-21

`REQ-014` R14 requires a ruling on whether a generated ideas-and-backlog HTML page is still wanted,
now that `IdeaExplorerRegion` and `BacklogExplorerRegion` both ship in the workbench (`phase-wb-06`,
complete), and states that declining the page and closing idea `000042` would satisfy it.
`phase-idg-08` owns that ruling, and `PLAN-036` R12 forbids any design phase from making it.

**The owner ruled on 2026-09-21 that the generated pages are wanted.** The interactive explorers and
the generated pages serve different needs: the explorers are live and interactive inside a running
workbench; the generated pages are shareable, snapshot-able, and readable without running the app.
Both surfaces survive. `phase-idg-08` executes this ruling — it records the kept decision and the
reason, and does not re-open the question.

Consequences for the idea log, all recorded there the same day:

- `000042` (generated ideas-and-backlog page) stays live and carries this ruling as an assessment
  annotation. It is not closed.
- `000300` (idea log and priority queue page) and `000301` (backlog and `next_up` page) are the
  2026-09-21 refinements of the same ask and are released by the same ruling.
- Neither page idea goes on the idea priority queue: `phase-idg-08` carries the work, and ranking it
  in two places would rank it twice. The owner ruled this explicitly.
- The owner did **not** rule on whether the pages are one surface or three. That stays open for
  whoever plans the work.

Two further asks captured in the same batch were deliberately held rather than released.
`000302` (prompt and plan explorer pages) and `000304` (Neo4j evaluation for graph-mapping the
relationships, including links down to code files) are both gated on `000303` (investigate the
relationships between ideas, backlog phases, prompts and plans), which the owner left captured and
unscheduled. The reason recorded for `000304` is that evaluating a graph store before establishing
which edges exist chooses a container before measuring the contents; the reason for `000302` is that
what an explorer should surface depends on what those edges turn out to be. Both gates and their
release condition are annotated on the ideas themselves, so a later triage or planning pass finds
them parked by decision rather than by oversight.
