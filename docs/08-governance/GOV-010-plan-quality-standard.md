...
with nothing after it. Lines inside a fenced code block do not count. Only one heading counts
toward two sections: `Requirement coverage`, explained under the plan table.

A plan **conforms** when every required section is present, and every conditional section is
present whenever its condition holds. A requirement conforms when the same holds for its sections
and its requirement rows pass the row check stated under the requirement table.
...
| Concurrency | when the plan's Work section lists two or more phases | `Execution order and real concurrency`, `Execution order` |
...
- **Does not.** The dynamic-HTML overview (`PLAN-003`) has a decisions table whose rationale column
  ...
  Tailwind is chosen for "Fast prototyping, consistent design system".
...
- **Meets it.** The idea-graph programme (`PLAN-029`) marks its first two decisions "**Ruled:**" and
  states the others as settled. The
...
- **Meets it.** The document-code plan (`PLAN-005`) gives the exact strings `--next-code` must
  ...
  an idea in a status the queue does not serve. `PLAN-026`'s
  requirement coverage table asserts coverage in both directions. `PLAN-034` explains, above its
  table, why `phase-proj-02` carries no row for R04.
...
- **Does not.** `PLAN-004`'s three open decisions are bare questions, with no owner, no deadline
  and no leaning. `PLAN-001`'s questions share one deadline, "before Phase 3", but name no one to
  decide them, and only one carries a leaning.
...
- **Meets it.** `PLAN-015` places each amendment in a dated block naming the owner and the session.
  Two plans meet it except for the date: `PLAN-016` states "**That is superseded.**" and gives the
  reason, and `PLAN-038` records that an earlier draft proposed comparing against `HEAD` alone and
  that the owner's ruling replaced it. Neither says when.
...
- **Meets it.** `PLAN-029` and `PLAN-026` open with a table defining each group code, such as
  `G01` to `G04`, before using it. Both then use programme codes such as `P4` and `P6` without
  defining them. That is the same failure on a smaller scale.
...
- **Meets it.** `PLAN-029`'s phase table has a "Depends on" column. `PLAN-018` states that steps
  3–6 "must land before or with steps 1–2", and gives the reason.
- **Does not.** `PLAN-002`'s implementation sequence has no dependency column and says "Each phase
  is independently useful". Yet its phase 4, the Session Briefing, reads the outputs of the signals
...
- **Does not.** `REQ-022` R01's verification begins "Inspect the orchestrator's graph definition",
  ...
  run's ledger entries against its gate decisions". Neither row says
  what result passes. `REQ-024` R01 names the passing and failing cases, but its command is written
  as `uv run python -c "..."`, so it cannot be run as written.
...
- **Too long.** `PLAN-001` is over 3,000 words. Most of it specifies JSON contracts, a 30-day
  proposal expiry and vector-store options for agents that do not yet exist. `PLAN-014`'s constraint names
...
These headings are not part of the mechanical check. Each is worth including only under the
stated condition. Each also has an example in the corpus of a plan that includes it without the
condition holding, which is what a copied-by-habit section looks like.
...
- **`Key references`** — only links the reader needs that the body does not already give, each
  with a note on what it is needed for.
  - Meets it: `PLAN-034`'s entries each say why the reader needs the link. For example, `ADR-009`
    is listed as "why `_data/` no longer answers `000022`'s question".
...
- **`Sizing against the partition`** — only in a plan derived from an idea partition, and only to
  ...
  - Does not: `PLAN-037`'s sizing section repeats, group by group, the phase counts its opening
    table already gives. Its partition gave no total to differ from.
- **`Deliverables`** — only when it names files and what each is for.
  - Meets it: `PLAN-016` lists each file with its purpose.
  - Does not: `PLAN-015`'s list opens with "This plan, as the standing policy". Its second item is
    a file it says was deleted.
