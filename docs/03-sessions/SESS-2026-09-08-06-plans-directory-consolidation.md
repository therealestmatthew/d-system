...
> `plans/` no longer exists (`ls plans/` empty). `docs/01-plans/PLAN-001-agent-memory-system.md` exists with front matter unchanged (`id: doc-agent-memory`, `code: PLAN-001`, `schema_version`, `owner`, `created`, `systems`, `depends_on` all identical to `plans/PLAN-001-agent-memory-system.md` as it stood at “Close phase-gov-03 via /session-close”). Diffing old vs. new shows exactly one changed line — the blockquote's two relative links (`../docs/08-governance/...` → `../08-governance/...` and same for `09-backlog`), correctly adjusted for the new directory depth. `docs/02-prompts/codex_governance_prompt.md` is byte-identical to the pre-move blob (`diff` produced no output).
>
...
> - The file list of “Consolidate root plans/ into docs/01-plans/ (phase-plc-01)” limited to `test/` returns nothing — no test files touched in the commit.
