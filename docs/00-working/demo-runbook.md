...
Both commands use the `env ...` form deliberately: it keeps the variables attached to the whole invocation, which survives being recalled from shell history. A bare `npm run dev -- --port 5180 --strictPort` recalled from history looks correct and starts a server on the right port with neither variable set (owner-confirmed, 2026-09-13).

**Verify the proxy before using the UI.** This must print `200`; `404` means the frontend is talking to the wrong backend:
```
curl -s -o /dev/null -w '%{http_code}\n' http://127.0.0.1:5180/api/v1/workbench/injection-sources
```

Without `VITE_API_TARGET` the dev proxy silently targets `http://127.0.0.1:8000` instead, and every `/api` route 404s if anything holds that port. Two symptoms appear together: the terminal slot reads "Terminal availability is unknown — the stage backend is not reachable", and all four injection dropdowns render as disabled buttons labelled "(terminal absent)" — greyed out, not openable, not merely empty. Both mean the same thing: the wrong backend, not a dead one. (Commands is disabled by the same terminal state but is not proxy-fed at all — it reads the static `/demo-commands.json` — so it is not diagnostic of the proxy.) Check what the running frontend process actually has, rather than what the launch command was meant to set:
```
