...
Without `VITE_API_TARGET` the dev proxy silently targets `http://localhost:8000` instead, and every `/api` route 404s if anything holds that port. Two symptoms appear together: the terminal slot reads "Terminal availability is unknown — the stage backend is not reachable", and the Commands/Skills/Prompts/Agents dropdowns are empty. Both mean the same thing — the wrong backend, not a dead one. Check what the running frontend process actually has, rather than what the launch command was meant to set:
```
tr '\0' '\n' < /proc/$(pgrep -f 'node.*vite --port 5180')/environ | grep -E 'D_SYSTEM|VITE_API'
```
Both lines must appear; empty output means the variables were dropped and the server needs restarting with the `env ...` form. (On Windows, use the Task Manager command-line column or restart rather than `/proc`.)
