...
**Coverage caveat, binding on this document.** This bibliography is built from the 67 rows this campaign deep-read into `04_evidence_matrix.csv` (30 of which are flagged `critical_collision: yes`), not the full candidate pool: 336 of 400 collision candidates surfaced by search have never been deep-read, including 2 of the 32 candidates in the top prescore band (band = max of the two prescores) -- `dhar-vaidhyanathan-varma-agenticakm-2026` and `epistemic-sybil-resistance-multiplying-agents-2026`, each a documented near-duplicate of a source this campaign did deep-read, under the partner ids `dhar-vaidhyanathan-varma-agenticakm-2026-arxiv` and `epistemic-sybil-resistance-bara-2026` respectively. The sources below are not represented as the strongest candidates the campaign could have found, only as the strongest candidates it actually read and the ones synthesis claims actually cite. No saturation claim is made anywhere in this document. Separately, and about a different pair from the near-duplicates just named: two source_id values in `03_source_inventory.csv` are each used on two separate inventory rows -- exact-duplicate ids, not near-duplicate content -- (`memtx-transactional-belief-commit-2026`, `semantically-seeded-graph-propagated-impact-analysis-vision-2026`); the inventory's row count is therefore not a distinct-source count, though neither pair affects the count below, since the evidence matrix (the source of Part A) already carries each underlying source once.

---

## Part A — Sources synthesis claims rest on

48 of the evidence matrix's 67 deep-read rows are cited by at least one claim in `07_anti_novelty_case.md`, `08_surviving_distinctions.md`, `09_reuse_recommendations.md`, `10_architecture_implications.md`, `11_open_research_questions.md` or `12_experiment_proposals.md` (re-derived from scratch against the current text of all six documents, using the same method as before: direct text search for each `source_id`, then, where a slug did not match verbatim, an author-name cross-check against the citing prose). The author-name cross-check rescued three rows whose slug never appears literally in any of the six documents: `levinson-et-al-fairscape-fair-reproducible-biomedical-analytics-2021` (cited as "Levinson" / "FAIRSCAPE" in `09_reuse_recommendations.md`), `krentsel-agarwal-cemri-reality-final-verifier-two-gaps-agentic-se-2026` (cited as "Krentsel" in `08_surviving_distinctions.md`, `11_open_research_questions.md` and `12_experiment_proposals.md`), and `maclean-young-bellotti-moran-qoc-design-space-analysis-1991` (cited as "MacLean, Young, Bellotti & Moran 1991" and via "QOC" in `07_anti_novelty_case.md` and `09_reuse_recommendations.md`). A fourth candidate rescue was checked and rejected: `conklin-begeman-gibis-hypertext-tool-cscw-1988` (Conklin & Begeman's gIBIS) is not counted as cited, because `07_anti_novelty_case.md`'s one mention of "IBIS" (line 262) names the general design-rationale tradition, not this paper or its authors by name -- neither "Conklin" nor "Begeman" nor "gIBIS" appears anywhere in `07`-`12`. The remaining 19 matrix rows -- `antoniol-recovering-traceability-links-tse-2002`, `arnold-bohner-software-change-impact-analysis-book-1996`, `capilla-et-al-web-based-tool-architectural-design-decisions-2006`, `chianti-cia-tool-java-icse-2005`, `claimtrust-propagation-trust-scoring-rag-2025`, `conklin-begeman-gibis-hypertext-tool-cscw-1988`, `ferrante-ottenstein-warren-program-dependence-graph-1987`, `inter-agent-trust-models-comparative-study-2025`, `iso-42010-2022-architecture-description-standard`, `iso-42010-conceptual-model-working-group-page`, `keim-kaplan-scattered-to-structured-akm-vision-2026`, `lineagerag-2026`, `mitigating-provenance-role-collapse-typed-memory-2026`, `perry-wolf-foundations-software-architecture-1992`, `prov-agent-2025`, `provenance-enhanced-statements-dec-2026`, `reasoning-belief-evidence-trust-multiagent-setting`, `uspto-10445654-feedforward-evidence-graph-confidence-patent`, and `weiser-program-slicing-tse-1984` -- were deep-read but are not drawn on by any synthesis claim; they remain in `04_evidence_matrix.csv` as read-but-unused evidence and are not listed here, per this document's scope (sources a synthesis claim *rests on*).
...
- `grading-narrators-isnad-rijal-claim-provenance-2026` -- Raja, A.Z.: Grading the Narrators: An Isnad-Rijal Framework for Claim-Level Provenance in Multi-Agent Knowledge Systems, arXiv preprint, 27 Jul 2026. (No affiliation given in `03_source_inventory.csv` or `04_evidence_matrix.csv`.)
  Identifier: `arxiv:2607.24117` | type: preprint | hypotheses: H1;H3;H4 | cc: yes | access: full_text

- `maclean-young-bellotti-moran-qoc-design-space-analysis-1991` -- MacLean, A., Young, R.M., Bellotti, V.M.E., Moran, T.P.: Questions, Options, and Criteria: Elements of Design Space Analysis, Human-Computer Interaction 6(3-4), 1991, pp.201-250.
  Identifier: `10.1207/s15327051hci0603&4_2` | type: peer-reviewed | hypotheses: NOT_APPLICABLE | cc: no | access: secondary_coverage

- `not-all-agreement-counts-as-corroboration-2026` -- Not All Agreement Counts as Corroboration: Provenance-Conserving Multi-View Fusion for Typed Action Admission in Human-Robot Collaboration, arXiv preprint, 2026. (No author name given in `03_source_inventory.csv` or `04_evidence_matrix.csv`.)
  Identifier: `arxiv:2609.01662` | type: preprint | hypotheses: H4 | cc: yes | access: full_text
...
- Total distinct validated/promoted bibliography entries in this document: **48** (Part A) **+ 6**
  (newly promoted seeds) **= 54** distinct sources, plus the 2 failed seeds recorded with their
  failing step named.
