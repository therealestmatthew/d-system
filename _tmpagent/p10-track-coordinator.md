# Track B — P10 programme execution (`phase-arch-*`)
...
|---|---|---|
| 1 | `01`, `11`, `14`, `16` | **2** — `01`+`11`, or `01`+`16` |
...
14 of the 17 idea-derived phases, and the validator treats a shared system as a collision. Adding
agents past two buys nothing except rejected claims.
...
## Run `phase-arch-00` first, alone

`phase-arch-00` decomposes `sys-ui` so the rest of the track stops serializing on one lock. The owner
directed on 2026-09-14 that it runs first, and the reason is concrete: every other `phase-arch-*`
phase would otherwise have to redeclare its `systems` afterwards.

It is the one phase where fan-out is actively wrong — it edits `systems.yaml`, which every peer's
claim reads. **One agent, no peers active, then re-measure with the snippet above** before planning
wave 1.
