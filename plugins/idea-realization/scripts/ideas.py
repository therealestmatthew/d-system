...
SCHEMA = paths.PLUGIN_ROOT / "schemas" / "idea.schema.json"

#: The one author whose annotations may be of any kind; every other author is an agent.
OWNER = "repository-owner"

#: The states an idea is worked in before it is promoted or closed.
WORKING_STATES = {"open", "triaged", "reviewing"}

#: Closes that require a `closes_with` pointer: where the delivery happened. With `discarded`
#: they are the terminal states — no outgoing status transition.
...
#: `link_retraction`, which always clears.
...
#: The fields a `classified` event may carry beyond its identity, time and author. Folded as
#: one unit: the latest `classified` event replaces the whole of the one before it, so an axis a
#: reclassification leaves out is not inherited from an older one.
...
#: The four classification axes a knowledge record carries a value on.
...
def identity(event: dict[str, Any]) -> str:
    ...
    An event written without an `eid` cannot gain one without rewriting an append-only log, so
    its identity is a pure function of bytes that are already permanent.
...
def _schema(schema: Path = SCHEMA) -> dict[str, Any]:
    data: dict[str, Any] = json.loads(schema.read_text(encoding="utf-8"))
    return data


def legal_transitions(schema: Path = SCHEMA) -> set[tuple[str, str]]:
    ...
    """
    for branch in _schema(schema)["allOf"]:
    ...
    raise IdeaError(f"{schema.name} declares no status transitions")
...
def _effective_fields(
    ...
    Because `_amendments_by_target` already proved every
...
def _as_promoted_to(value: str | list[str] | None) -> list[str] | None:
    """Normalise `promoted_to` to a list. A scalar reads as a one-item list."""
    if value is None:
        return None
...
def fold(events: list[dict[str, Any]]) -> dict[str, dict[str, Any]]:
    ...
    permitted regardless of status, terminal included: they extend the record,
    not the state machine. The latest `classified` event is the idea's classification.

    Some keys appear only where the data does — an idea's `closes_with` once it is closed, its
    `classification` once classified, a link's `target_code` when it points at a document — so an
    idea that never used them folds to the same state it always did. Read them with `.get()`.
    """
...
def link_diagnostics(state: dict[str, dict[str, Any]]) -> dict[str, list[str]]:
    """Fold-time diagnostics, keyed by the idea each is reported against.

    Never raises: a cycle or a stale supersession is a modelling mistake worth surfacing, not a
    reason to refuse the append that revealed it: capture always wins, for links as for ideas.
    Two checks: an `extends` cycle is flagged against every idea on it, and a `supersedes` edge
    whose target is not `discarded` is flagged against the idea asserting it.
    """
