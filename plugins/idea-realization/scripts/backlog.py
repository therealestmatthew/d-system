...
# Stale-claim threshold: double the longest claim-to-completion gap observed where this rule was
# first measured (one day), so an ordinary session in flight is never flagged.
STALE_CLAIM_DAYS = 2
...
def stale_claim_signal(days_since_commit: int | None, worktree_exists: bool | None) -> str | None:
    ...
    a moment ago" as with "abandoned before ever starting," and the second case is for a
    person to judge, not this function.
    """
...
def claim_report_state(evidence: dict[str, Any] | None) -> str:
    ...
    - `evidence` present but both fields are `None`: no commit was found on
      ...
      `agent/<phase-id>` naming convention — the check has nothing to evaluate,
...
def inspect_backlog(
    ...
    # Optional: a fresh repository has no decisions document. When named, it must resolve.
    if "decision_record" in catalog:
