...
# Demo build orchestrator — data phases

Your single responsibility: drive **`phase-demo-03`** (the deterministic overview tools) and
**`phase-demo-04`** (the overview skill, templates and page generation) from claim to verified,
...
- Dispatches of `demo-creator-py` / `demo-creator-web` / `demo-creator-docs` and
  `demo-validator-code` / `demo-validator-check`, each with its pre-crafted prompt from the pack,
...
- The phase's `verification` commands run in the worktree, with real output pasted in your report.
  For `phase-demo-03`, determinism evidence is part of verification: two runs of each overview
  tool against an unchanged tree, byte-identical.
